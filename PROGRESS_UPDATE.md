...
- **Marketplace redesign proposal** — `docs/MARKETPLACE_DESIGN_PROPOSAL.md` (2026-09-26, Sandile.Codex, senior response recorded). Real bug found and prioritised for fixing first: marketplace search never actually filters as you type (`searchQuery` is a plain field read inside an Angular `computed()`, which only tracks signal reads — confirmed). Also: a second WCAG contrast failure (`#2DB344` price/success green on white, ≈2.75:1 — confirmed, likely app-wide, not just marketplace), community filtering wired on the backend but never used by the marketplace UI, seller name surfaced on the card, category-tinted photo placeholders, and a logo-simplification option. Sandile's direction on the open questions is still pending.

...
- Verified live: rebuilt frontend + backend containers, confirmed both serve HTTP 200. The product grid layout itself was already responsive and mobile-first (2-col base → 3-col at 480px → auto-fill at 768px+) — only the reactivity was broken, no CSS changes needed.

### Marketplace demo product seeding (2026-09-26)
- Added 20 realistic marketplace listings via `DataInitializer.seedMarketplaceDemoProducts()`, spread across all 5 seeded communities and all 8 categories (previously only 3 hand-created products existed, all in one community) — enough real density to actually evaluate the marketplace frontend and community-pill filtering against, per the ongoing `docs/MARKETPLACE_DESIGN_PROPOSAL.md` review.
- 4 new demo sellers created (one each in KwaNibela, KwaMakhasa, KwaJobe, KwaMnqobokazi, the 4 communities that had zero businesses before); the remaining new products attached to the 3 existing hand-created sellers (resolved by anchoring on known product names rather than business name, since this dev database already has duplicate "Liskomz Tuck Shop" rows from earlier manual testing).
- Every product — new and the 2 pre-existing photo-less ones (Vetkoek, Coke 330ml) — now has an image. **No image-generation tool is available in this environment**, so these are 17 flat illustrated icons (market-stall-logo style: simple shapes, brand palette, on the app's existing `--bg-muted` tan tone) rather than photographs, reused across similar products (e.g. one "leafy greens" icon for spinach and butternut). Served through the real `/api/uploads/{filename}` path, copied into the `uploads_data` volume — **the image files themselves are not in git** (matching how all uploads work); `scripts/generate-marketplace-seed-icons.js` regenerates them from source if the volume is ever lost, with the exact `docker cp` command in the script's header comment.
- Verified end-to-end through the real stack, not just the DB: rebuilt via `docker compose up --build`, confirmed all 23 products via `GET /api/products`, confirmed a seed image resolves with HTTP 200 through the frontend's own nginx proxy at `/api/uploads/...`, confirmed community distribution (KwaNgwenya 9, KwaNibela 4, KwaMakhasa 4, KwaJobe 3, KwaMnqobokazi 3).
- Idempotent: guarded on whether "KwaNibela Green Grocer" already exists, so this never re-seeds or duplicates on container restart.

### UI/UX audit fixes, dark/light theme, English/isiZulu language switch (2026-09-23)
- **Audit fixes** (see `docs/UI_UX_AUDIT_CLAUDE.md` for the full findings list): fixed the yellow-on-white contrast failure on the toolbar login link and "Join thenga.com" menu item (now `var(--accent-text-safe)`, `#92620A` light / `#F5D061` dark); added the missing `aria-label` on the sidenav close button; fixed "HUSTLE" → "thenga.com" in the hustler dashboard's desktop rail; fixed the facilitator sign-out button's corner-radius drift; fixed the income/expense chart legend dots to use the actual brand green/red tokens instead of generic Tailwind colours; added a single global `prefers-reduced-motion` rule; deleted three orphaned, unrouted auth page components (`register-page`, `customer-login-page`, `customer-register-page`); resolved the conflicting `.offline-bar` definitions (global orange/white now applies, the component-local amber override is gone); replaced all eleven `transition: all` instances with named properties; rebuilt `app-select` (used by nearly every dropdown in the app) with real accessibility — `role="combobox"`/`listbox`/`option`, `aria-expanded`, `aria-activedescendant`, and full arrow-key/Home/End/Enter/Escape keyboard support.
- **Not done in this pass, tracked as follow-ups:** consolidating the marketplace/business-page/hustler-dashboard product card into one shared component; replacing emoji/glyph icons app-wide with a real icon set (only the new sidenav theme/language toggles use `mat-icon` instead of emoji).
- **Dark / light mode**: `ThemeService` (signal-based, `localStorage`-persisted, falls back to `prefers-color-scheme` on first load) sets `data-theme` on `<html>`; `styles.css` now defines dark equivalents for every surface/text/border/shadow token under `:root[data-theme="dark"]`. Toggle switch lives in the sidenav. Fully theme-aware: the app shell (toolbar/sidenav/bottom nav), the marketplace, the login/register page, and checkout. **Not yet converted**: the hustler dashboard, facilitator/coordinator/operations tools, and the driver dashboard still hardcode light-theme hex colours locally, so their cards will render as light content on a dark page background rather than fully dark — a legitimate "dark chrome, light content" v1 look, not a bug, but full coverage is a larger follow-up (same root cause as the token-consolidation note in the audit).
- **English / isiZulu language switch**: `TranslationService` (signal-based, `localStorage`-persisted) + a `translate` pipe, with an embedded dictionary rather than a fetched JSON asset (works offline). Toggle switch lives in the sidenav next to the theme toggle. Covers the app shell, marketplace category labels/search/empty-states, login/register labels, and the main checkout labels. **Not yet translated**: everything else — the hustler dashboard, the entire facilitator/coordinator/operations pipeline, the driver dashboard, survey forms, and all validation/error messages, which remain English-only. **The isiZulu text is an AI best-effort translation, not reviewed by a fluent speaker — do not extend this pattern to legally significant copy (e.g. the hustler agreement terms) without native-speaker review first**, and the existing UI strings should get that review before this ships to real users.
...
- **Logo replaced (2026-09-23)** — see the "UI/UX audit fixes, dark/light theme..." entry above; theme colours left as-is pending a brand palette

### 📐 Design specs on deck (drafted, NOT built)
- **Facilitator-Seller role + earnings ledger** — `docs/FACILITATOR_SELLER_SPEC.md`. Paid youth-employment layer: earn per verification / onboarded seller / assisted transaction, on top of running a store. New `FACILITATOR_SELLER` role, append-only `FacilitatorEarning` ledger, `EarningRate`, `PayoutBatch`. Decisions recorded 2026-09-11 (spec §8, `CODE_REVIEWS.md` DL-1): flat fees, coordinator-granted role, platform wallet with Flash cash-out. Funding source still open.
- **Last-mile parcel relay** — `docs/LAST_MILE_DELIVERY_SPEC.md`. Local drivers bridge the gap national couriers won't (Mkuze depot → rural home). Hub-logged parcels, consolidated multi-drop `Trip`s, cash-on-delivery zone tariffs. New `Hub`/`Parcel`/`Trip`/`DeliveryTariff` entities (parallel to `DeliveryJob`, not a rewrite). Hubs run by the Facilitator-Seller layer; reuses the earnings/payout machinery. Decisions recorded 2026-09-11 (spec §8, `CODE_REVIEWS.md` DL-1): consent before assignment, flat zone tariff, per-trip driver→hub cash, guest recipients, one hub per trip.
- **Marketplace redesign proposal** — `docs/MARKETPLACE_DESIGN_PROPOSAL.md` (2026-09-26, Sandile.Codex, senior response recorded). Top-priority bug (search never filtered as you type) is now **fixed** — see "Marketplace search fix" above. Still open: a second WCAG contrast failure (`#2DB344` price/success green on white, ≈2.75:1 — confirmed, likely app-wide, not just marketplace), community filtering wired on the backend but never used by the marketplace UI, seller name surfaced on the card, category-tinted photo placeholders, and a logo-simplification option. Sandile's direction on these remaining open questions is still pending.
